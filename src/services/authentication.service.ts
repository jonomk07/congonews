...
export class AuthenticationService {
  ...
  doLogin(username, password){
    return this.http.post(Config.WORDPRESS_URL + 'wp-json/jwt-auth/v2/token',{
      username: username,
      password: password
    })
  }
  ...
  validateAuthToken(token){
    ...
    return this.http.post(Config.WORDPRESS_URL + 'wp-json/jwt-auth/v2/token/validate?token=' + token,
      {}, {headers: header})
  }
}
